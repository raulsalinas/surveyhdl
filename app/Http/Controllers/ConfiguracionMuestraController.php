...
class ConfiguracionMuestraController extends Controller
{
    public function listar(Request $request)
    {
        $data = Muestra::withTrashed()->select('muestra.*'
        )->with('fecha','personal');
        ...
        ->addColumn('accion', function ($data) { return 
            '<div class="btn-group" role="group">
                <button type="button" class="btn btn-xs btn-warning editar-muestra" data-id="'.$data->id.'" title="Editar muestra" ><i class="fa-solid fa-pencil"></i></button>
            </div>';
        })
    ...
    public function obtener($id){
        try {
        $error = "";
        $data= Muestra::withTrashed()->with('fecha','personal')->find($id);
        if(empty($data)==false){
            $status = 'info';
    ...
    public function guardar(Request $request)
    {
        ...
        // try {
            ...
            //     $error = "";
            // } else {
                $data = Muestra::withTrashed()->firstOrNew(['id' => intval($request->id)]);
